...
class ChatGUI:
    def __init__(self, bot):
        ...
        # Header Section
        header_frame = tk.Frame(self.window, bg="#1e1e1e" if self.dark_mode else "#f0f0f0")
        header_frame.pack(pady=(10, 0))

        try:
            logo_img = Image.open("logo.png").resize((40, 40))
            self.logo_photo = ImageTk.PhotoImage(logo_img)
            logo_label = tk.Label(header_frame, image=self.logo_photo, bg=header_frame["bg"])
            logo_label.pack(side=tk.LEFT, padx=10)
        except:
            pass

        title_label = tk.Label(header_frame, text="RuleBot 🤖", font=("Segoe UI", 16, "bold"),
                               fg="white" if self.dark_mode else "black", bg=header_frame["bg"])
    ...
    def send_message(self, event=None):
        ...
        if user_msg.lower() in self.bot.exit_commands:
            farewell = "Okay, have a nice Earth day! Goodbye!"
            self.display_message(farewell, sender="bot")
            self.window.after(100, lambda: self.speak(farewell))
